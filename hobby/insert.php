 ...
 $pdo = db_connect();
  ...
  if ($mode=="modify"){
      ...
      try{
         $sql = "select * from yu14db.hobby where num=?";  // get target record
      ...
      for ($i=0; $i<$count; $i++)						
      {
		...
		unlink($delete_path);
		   ...
		   $pdo->beginTransaction(); 
                   $sql = "update yu14db.hobby set $field_org_name = ?, $field_real_name = ?  where num=?";
                   $stmh = $pdo->prepare($sql); 
     ...
     try{
        $pdo->beginTransaction();   
        $sql = "update yu14db.hobby set subject=?, content=?, is_html=? where num=?";
        $stmh = $pdo->prepare($sql); 
   ...
   try{
     $pdo->beginTransaction();
     $sql = "insert into yu14db.hobby(id, name, nick, subject, content, regist_day, hit, is_html, ";
   ...
   header("Location:http://localhost/YUproject/hobby/list.php");
 ?>
