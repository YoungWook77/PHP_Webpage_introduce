 ...
 $pdo = db_connect();
  ...
  if ($mode=="modify"){
      ...
      try{
         $sql = "select * from yu14db.secinfo where num=?";  // get target record
      ...
      for ($i=0; $i<$count; $i++)						
      {
		...
		unlink($delete_path);
		   ...
		   $pdo->beginTransaction(); 
                   $sql = "update yu14db.secinfo set $field_org_name = ?, $field_real_name = ?  where num=?";
                   $stmh = $pdo->prepare($sql); 
     ...
     try{
        $pdo->beginTransaction();   
        $sql = "update yu14db.secinfo set subject=?, content=?, is_html=? where num=?";
        $stmh = $pdo->prepare($sql); 
   ...
   try{
     $pdo->beginTransaction();
     $sql = "insert into yu14db.secinfo(id, name, nick, subject, content, regist_day, hit, is_html, ";
   ...
   header("Location:http://localhost/YUproject/secinfo/list.php");
 ?>
