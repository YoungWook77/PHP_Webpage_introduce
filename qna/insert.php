 ...
 if ($mode=="modify"){
   try{
     ...
     $sql = "update yu14db.qna set subject=?, content=?, is_html=? where num=?";
     ...
     header("Location:http://localhost/YUproject/qna/list.php?page=$page");
    ...
    {
     try{
       $sql = "select * from yu14db.qna where num = $num"; // 부모 글 가져오기
       $stmh = $pdo->prepare($sql);  
	...
	// ord 가 부모글의 ord($row[ord]) 보다 큰 경우엔 ord 값 1 증가 시킴
       $pdo->beginTransaction();
       $sql = "update yu14db.qna set ord = ord + 1 where group_num = ? and ord > ?";
       $stmh = $pdo->prepare($sql); 
       ...
       $pdo->beginTransaction();  
       $sql = "insert into yu14db.qna(group_num, depth, ord, id, name, nick, subject,";
     ...
     try{
       ...
       $sql = "insert into yu14db.qna(depth,ord,id,name,nick,subject,content,regist_day,hit,is_html) ";
       $sql .= "values(?, ?, ?, ?, ?, ?, ?,now(), 0, ?)";
       $stmh = $pdo->prepare($sql); 
       ...
       $pdo->beginTransaction();   
       $sql = "update yu14db.qna set group_num = ? where num=?";
