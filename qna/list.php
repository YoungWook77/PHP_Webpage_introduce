 ...
 <?php
   ...
   $sql="select * from yu14db.qna where $find like '%$search%' order by num desc";
  } else {
   $sql="select * from yu14db.qna order by group_num desc, ord asc limit $first_num, $scale";
   //limit를 사용해 레코드 개수를 한 페이지당 출력하는 수로 제한
  }
 try{
  $stmh = $pdo->query($sql); 
  
  $sql = "select * from yu14db.qna";  //전체 레코드수를 파악하기 위함.
  $stmh1 = $pdo->query($sql);
